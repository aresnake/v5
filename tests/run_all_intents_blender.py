...
# ” Recharge force du resolver corrig
...
def resolve_config_path():
    candidates = [
        os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "config", "voice_config.yaml")),
        os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "config", "voice_config.yaml")),
    ]
...
def main():
    ...
    phrases = [block["phrase"] for block in config.values() if isinstance(block, dict) and "phrase" in block]
    for phrase in phrases:
        log.info(f"” Test de la phrase : {phrase}")
        result = run_pipeline(phrase, mode="test")
        log.info(f"“ Rsultat : {result}")
