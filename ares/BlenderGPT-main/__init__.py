...
class GPT4_OT_Execute(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not openai.api_key:
            self.report(
                {'ERROR'}, "No API key detected. Please set the API key in the addon preferences."
            )
            return {'CANCELLED'}
        ...
        blender_code = generate_blender_code(
            context.scene.gpt4_chat_input, context.scene.gpt4_chat_history, context, system_prompt
        )
