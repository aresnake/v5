...
def list_species() -> Dict[str, dict]: return _species
def list_styles() -> Dict[str, dict]:  return _styles
...
def generate_spider(species_key: str, style_key: Optional[str] = None, provider: str = "offline_stub") -> str:
    if species_key not in _species: raise ValueError(f"Unknown species: {species_key}")
    ...
    options = {"camera_fov": style.get("camera_fov", 35),
               "lighting": style.get("lighting", ""),
               "texture_resolution": style.get("texture_resolution", "2K")}
    mng = _get_manager()
    if not mng: raise RuntimeError("Text2Mesh manager not available.")
